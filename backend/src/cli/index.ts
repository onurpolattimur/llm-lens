...
function addInspectorOptions(command: Command): Command {
  return command
    .option("--proxy-port <port>", "proxy port", envValue("LLM_LENS_PROXY_PORT", DEFAULT_PROXY_PORT, "LLM_INSPECTOR_PROXY_PORT"))
    .option("--api-port <port>", "API/WebSocket port", envValue("LLM_LENS_API_PORT", DEFAULT_API_PORT, "LLM_INSPECTOR_API_PORT"))
    .option("--ui-port <port>", "web UI port", envValue("LLM_LENS_UI_PORT", DEFAULT_UI_PORT, "LLM_INSPECTOR_UI_PORT"))
    .option("--host <host>", "bind host", envValue("LLM_LENS_HOST", DEFAULT_HOST, "LLM_INSPECTOR_HOST"))
    .option("--additional-provider-urls <urls>", "extra provider hosts or URLs to capture", envValue(ADDITIONAL_PROVIDER_URLS_ENV, "", "LLM_INSPECTOR_ADDITIONAL_PROVIDER_URLS"))
...
function envValue(key: string, fallback: string, legacyKey?: string): string {
  return process.env[key] || (legacyKey ? process.env[legacyKey] : undefined) || fallback;
}
