...
LLM Inspector is a local debugging proxy and web UI for inspecting LLM API traffic.
...
The project works like a local MITM proxy: start LLM Inspector, point your tool at
...
Inspector parses provider payloads into a normalized conversation trace with
...
LLM Inspector captures these provider hosts by default:
...
You can add more hosts with `LLM_INSPECTOR_ADDITIONAL_PROVIDER_URLS` or the
`--additional-provider-urls` CLI flag. Extra hosts are captured as `unknown`
unless their payload shape matches a parser that LLM Inspector already
understands.
...
export REQUESTS_CA_BUNDLE="$HOME/.llm-inspector/certs/certs/ca.pem"
```
...
NODE_EXTRA_CA_CERTS="$HOME/.llm-inspector/certs/certs/ca.pem" \
SSL_CERT_FILE="$HOME/.llm-inspector/certs/certs/ca.pem" \
your-agent-command
```
...
```sh
npx llm-inspector --proxy-port 9191 opencode --help
```
...
Run an agent command through LLM Inspector:
...
```sh
npx llm-inspector cert generate
...
certificate files remain under the LLM Inspector home directory.

## Configuration

LLM Inspector reads `.env.local` and `.env` from the directory where the command
...
LLM_INSPECTOR_HOME="$HOME/.llm-inspector"
LLM_INSPECTOR_ADDITIONAL_PROVIDER_URLS=api.example.com,llm.internal.test
```

`LLM_INSPECTOR_ADDITIONAL_PROVIDER_URLS` accepts comma-separated or
whitespace-separated hosts and URLs.

## Privacy and Security

LLM Inspector is designed to run locally. By default it binds to `127.0.0.1`,
...
```sh
$HOME/.llm-inspector/certs/keys/ca.private.key
```
...
```sh
$HOME/.llm-inspector/certs/certs/ca.pem
```
...
- Debian/Ubuntu: copy `ca.pem` to
  `/usr/local/share/ca-certificates/llm-inspector-ca.crt`, then run
